...
- **白板**：可建立**多個白板**，每個白板放入你自己挑選的卡片子集（同一張卡可出現在多個白板、各有位置）。無限畫布，拖曳卡片、平移、縮放（滑鼠滾輪 / 雙指捏合）；卡片間以貝茲曲線連線（實線＝已確認、紫色虛線＝AI 建議）。可在白板上新建卡片或從卡片庫加入既有卡片。白板數量變多時，分頁可直接拖曳排序，用不到的可以**封存**（移出分頁列、不刪任何卡片），排序與封存都會同步到其他裝置；「管理白板」對話框可搜尋、排序、封存與還原。
...
`order`（分頁列順序）與 `archived`（已封存）只有在實際排序／封存過之後才會寫入，沒動過的白板檔維持原樣。
